...
func parseFlags() BMCConfig {
	...
	flag.StringVar(&config.Endpoint, "endpoint", os.Getenv("BMC_ENDPOINT"), "BMC endpoint URL (or set BMC_ENDPOINT env var)")
	flag.StringVar(&config.Username, "username", os.Getenv("BMC_USERNAME"), "BMC username (or set BMC_USERNAME env var)")
	flag.StringVar(&config.Password, "password", os.Getenv("BMC_PASSWORD"), "BMC password (or set BMC_PASSWORD env var)")
	flag.StringVar(&config.Vendor, "vendor", getEnvWithDefault("BMC_VENDOR", "generic"), "BMC vendor: dell, hpe, lenovo, or generic")
...
Environment Variables:
  BMC_ENDPOINT   BMC URL (e.g., https://192.168.1.100)
  BMC_USERNAME   BMC username
  BMC_PASSWORD   BMC password
  BMC_VENDOR     BMC vendor (dell, hpe, lenovo, generic)
