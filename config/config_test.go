...
func TestMetricsLevelValue_Set(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			initialLevel:  metrics.MetricsLevelAll,
			setValue:      "node",
		...
		{
			name:          "Set all explicitly",
			initialLevel:  metrics.MetricsLevelNone,
			setValue:      "all",
			expectedLevel: metrics.MetricsLevelAll,
			expectError:   false,
		},
		{
			name:          "Set none explicitly",
			initialLevel:  metrics.MetricsLevelAll,
			setValue:      "none",
			expectedLevel: metrics.MetricsLevelNone,
			expectError:   false,
		},
		{
		...
		{
			name:          "Set all overrides existing levels",
			initialLevel:  metrics.MetricsLevelNode | metrics.MetricsLevelProcess,
			setValue:      "all",
			expectedLevel: metrics.MetricsLevelAll,
			expectError:   false,
		...
		{
			name:          "Set none overrides existing levels",
			initialLevel:  metrics.MetricsLevelNode | metrics.MetricsLevelProcess,
			setValue:      "none",
			expectedLevel: metrics.MetricsLevelNone,
			expectError:   false,
		},
		{
			...
			initialLevel:  metrics.MetricsLevelAll,
			setValue:      "invalid",
			expectedLevel: metrics.MetricsLevelAll, // Should remain unchanged
			expectError:   true,
...
func TestMetricsLevelValue_AccumulativeBehavior(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:          "Set all after individual levels",
			initialLevel:  metrics.MetricsLevelNone,
			setValues:     []string{"node", "process", "all"},
			expectedLevel: metrics.MetricsLevelAll,
			expectError:   false,
		},
		{
			name:          "Set none after individual levels",
			initialLevel:  metrics.MetricsLevelNone,
			setValues:     []string{"node", "process", "none"},
			expectedLevel: metrics.MetricsLevelNone,
			expectError:   false,
		},
		{
			name:          "Set all then individual (should reset then accumulate)",
			initialLevel:  metrics.MetricsLevelNone,
			setValues:     []string{"all", "node"},
			expectedLevel: metrics.MetricsLevelNode,
			expectError:   false,
		},
		{
...
func TestMetricsLevelValue_String(t *testing.T) {
	tests := []struct {
		...
		{
			name:     "None level",
			level:    metrics.MetricsLevelNone,
			expected: "none",
		},
		{
			name:     "All level",
			level:    metrics.MetricsLevelAll,
			expected: "all",
		},
		{
		...
		{
			name:     "All individual levels combined",
			...
			expected: "all", // Should be equivalent to all
		},
	}
...
func TestMetricsLevelValue_CommandLineIntegration(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:          "Multiple flag values with all",
			args:          []string{"--metrics", "node", "--metrics", "all"},
			expectedLevel: metrics.MetricsLevelAll,
			expectError:   false,
		},
		{
			name:          "Multiple flag values with none",
			args:          []string{"--metrics", "node", "--metrics", "none"},
			expectedLevel: metrics.MetricsLevelNone,
			expectError:   false,
		},
		{
		...
		},
		{
			...
			expectedLevel: metrics.MetricsLevelAll, // Should remain at default
			expectError:   true,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.expectError {
				...
				assert.Equal(t, metrics.MetricsLevelAll, metricsLevel)
			} else {
				assert.NoError(t, err)
