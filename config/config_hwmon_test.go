...
func TestApplyHwmonFlags(t *testing.T) {
	tests := []struct {
		...
		hwmon: &Hwmon{},
		flagsSet: map[string]bool{
			ExperimentalHwmonChipsFlag: true,
		},
		enabled: ptr.To(true),
		zones:   &[]string{"package", "core"},
		chips:   &[]string{"k10temp", "amdgpu"},
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			applyHwmonFlags(tc.hwmon, tc.flagsSet, tc.enabled, tc.zones, tc.chips)
...
func TestApplyHwmonConfig(t *testing.T) {
	tests := []struct {
		...
		name: "chips flag overrides config",
		cfg: &Config{
			Experimental: &Experimental{
				Hwmon: Hwmon{
					Chips: []string{"old-chip"},
				},
			},
		},
		flagsSet: map[string]bool{
		...
		enabled: ptr.To(false),
		zones:   &[]string{},
		chips:   &[]string{"new-chip1", "new-chip2"},
	}, {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			err := applyHwmonConfig(tc.cfg, tc.flagsSet, tc.enabled, tc.zones, tc.chips)
			...
			if hasHwmonFlags(tc.flagsSet) {
				...
				// Verify chips were applied if set
				if tc.flagsSet[ExperimentalHwmonChipsFlag] {
					assert.Equal(t, *tc.chips, tc.cfg.Experimental.Hwmon.Chips)
				}
			}
		})
	}
...
func TestIsFeatureEnabled_Hwmon(t *testing.T) {
	tests := []struct {
		...
		name: "hwmon feature with zones and chips but disabled",
		config: &Config{
			Experimental: &Experimental{
				Hwmon: Hwmon{
					Enabled: ptr.To(false),
		...
		name: "hwmon feature enabled with zones and chips",
		config: &Config{
			Experimental: &Experimental{
				Hwmon: Hwmon{
					Enabled: ptr.To(true),
...
func TestSanitize_HwmonFields(t *testing.T) {
	tests := []struct {
		...
		expectedChips: []string{"k10temp", "amdgpu"},
	}, {
		name: "empty zones and chips",
		config: &Config{
			Experimental: &Experimental{
		...
		name: "zones and chips with tabs and newlines",
		config: &Config{
			Experimental: &Experimental{
				Hwmon: Hwmon{
					Enabled: ptr.To(true),
		...
		name: "nil zones and chips slices",
		config: &Config{
			Experimental: &Experimental{
				Hwmon: Hwmon{
					Enabled: ptr.To(true),
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			assert.Equal(t, tc.expectedChips, tc.config.Experimental.Hwmon.Chips)
		})
	}
}
...
`,
		...
		validateFunc: func(t *testing.T, cfg *Config) {
			...
			assert.Equal(t, []string{"k10temp", "amdgpu"}, cfg.Experimental.Hwmon.Chips)
		},
	}, {
...
`,
		...
		name: "hwmon with empty zones and chips",
		yamlContent: `
experimental:
  hwmon:
    enabled: true
...
`,
		...
		validateFunc: func(t *testing.T, cfg *Config) {
			...
			assert.Empty(t, cfg.Experimental.Hwmon.Chips)
		},
	}, {
