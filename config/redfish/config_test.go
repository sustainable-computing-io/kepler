...
func TestResolveNodeID(t *testing.T) {
	tt := []struct {
		...
		redfishNodeID:  "redfish-node",
		kubeNodeName:   "kube-node",
		expectedResult: "redfish-node",
		expectError:    false,
	}, {
		name:           "Kubernetes node name fallback",
		redfishNodeID:  "",
		kubeNodeName:   "kube-node",
		expectedResult: "kube-node",
		expectError:    false,
	}, {
		name:          "Hostname fallback",
		redfishNodeID: "",
		kubeNodeName:  "",
		expectError:   false, // Will use hostname
	}, {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.expectError {
				...
				if tc.expectedResult != "" {
					...
					assert.NotEmpty(t, result) // For hostname fallback
				}
			}
		})
	}
