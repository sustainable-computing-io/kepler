...
#define BPF_SNPRINTF(out, out_size, fmt, args...)                      \
	({                                                             \
		...
		_Pragma("GCC diagnostic pop")                          \
                                                                       \
			bpf_snprintf(out, out_size, ___fmt, ___param,  \
				     sizeof(___param));                \
	})
...
extern int
bpf_iter_num_new(struct bpf_iter_num *it, int start, int end) __weak __ksym;
...
# define bpf_for_each(type, cur, args...)                                                                                                                                                        \
	 for (/* initialize and define destructor */                                                                                                                                             \
	      ...
	      *___p __attribute__((unused)) =                                                                                                                                                    \
		      (bpf_iter_##type##_new(                                                                                                                                                    \
			       &___it, ##args), /* this is a workaround for Clang bug: it currently doesn't emit BTF */ /* for bpf_iter_##type##_destroy() when used from cleanup() attribute */ \
		       (void)bpf_iter_##type##_destroy,                                                                                                                                          \
		       (void *)0); /* iteration and termination check */                                                                                                                         \
...
# define bpf_for(i, start, end)                                                                                                                                                    \
	 for (/* initialize and define destructor */                                                                                                                               \
	      ...
	      *___p __attribute__((unused)) =                                                                                                                                      \
		      (bpf_iter_num_new(                                                                                                                                           \
			       &___it, (start),                                                                                                                                    \
			       (end)), /* this is a workaround for Clang bug: it currently doesn't emit BTF */ /* for bpf_iter_num_destroy() when used from cleanup() attribute */ \
