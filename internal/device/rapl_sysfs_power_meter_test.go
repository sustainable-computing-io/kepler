...
func TestAggregatedZoneIntegration(t *testing.T) {
	...
	mockReader := &mockSysFSReader{
		response: []EnergyZone{
			// Two package zones and one core zone
			mockZone{name: "package-0", index: 0, path: "/intel-rapl:0", energy: 1000, maxEnergy: 100000},
			mockZone{name: "package-1", index: 1, path: "/intel-rapl:1", energy: 2000, maxEnergy: 100000},
			mockZone{name: "core-0", index: 0, path: "/intel-rapl:0:0", energy: 500, maxEnergy: 50000},
		},
	}
	...
	for _, zone := range zones {
		if zone.Name() == "package" {
			packageZone = zone
		} else if zone.Name() == "core-0" { // Single zone keeps original name
			coreZone = zone
		}
	}
