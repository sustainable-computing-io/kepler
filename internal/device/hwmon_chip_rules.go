...
// It checks configRules first (user overrides/additions), then falls back to hardcoded defaults.
func getChipPairingRule(chipName string, configRules []ConfigChipRule) *chipPairingRule {
	// Normalize chip name for lookup
	normalized := strings.ToLower(strings.TrimSpace(chipName))

	// Check config rules first (user overrides/additions take precedence)
	for _, cr := range configRules {
		if strings.ToLower(strings.TrimSpace(cr.Name)) == normalized {
			return &chipPairingRule{
