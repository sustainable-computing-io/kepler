...
func TestHwmonPowerMeter_ZoneDetails(t *testing.T) {
	...
	for _, zone := range zones {
		...
		} else if calcZone, ok := zone.(*hwmonVoltageCurrentZone); ok {
...
// =============================================================================
// Tests for hwmonVoltageCurrentZone (voltage/current based power calculation)
// =============================================================================
...
func TestHwmonCalculatedPowerZone_Power(t *testing.T) {
	t.Logf("\n=== Testing hwmonVoltageCurrentZone.Power() ===")
...
func TestHwmonCalculatedPowerZone_Methods(t *testing.T) {
	t.Logf("\n=== Testing hwmonVoltageCurrentZone Interface Methods ===")
...
func TestHwmonCalculatedPowerZone_Power_VoltageReadError(t *testing.T) {
	t.Logf("\n=== Testing hwmonVoltageCurrentZone.Power() Voltage Read Error ===")
...
func TestHwmonCalculatedPowerZone_Power_CurrentReadError(t *testing.T) {
	t.Logf("\n=== Testing hwmonVoltageCurrentZone.Power() Current Read Error ===")
...
func TestHwmonCalculatedPowerZone_Power_InvalidVoltageContent(t *testing.T) {
	t.Logf("\n=== Testing hwmonVoltageCurrentZone.Power() Invalid Voltage Content ===")
...
func TestHwmonCalculatedPowerZone_Power_InvalidCurrentContent(t *testing.T) {
	t.Logf("\n=== Testing hwmonVoltageCurrentZone.Power() Invalid Current Content ===")
...
func TestDiscoverVoltageCurrentZones_MatchByLabel(t *testing.T) {
	...
	calcZone, ok := zone.(*hwmonVoltageCurrentZone)
	require.True(t, ok, "Zone should be *hwmonVoltageCurrentZone type")
...
func TestDiscoverZones_FallbackToVoltageCurrentWhenNoPower(t *testing.T) {
	...
	for i, zone := range zones {
		...
		// Verify it's a calculated power zone
		_, ok := zone.(*hwmonVoltageCurrentZone)
		assert.True(t, ok, "Zone should be *hwmonVoltageCurrentZone type")
	}
...
func TestHwmonCalculatedPowerZone_MultipleReadings(t *testing.T) {
	t.Logf("\n=== Testing hwmonVoltageCurrentZone Multiple Readings ===")
...
func TestGetChipPairingRule(t *testing.T) {
	...
	for _, tc := range testCases {
		rule := getChipPairingRule(tc.chipName, nil)
		if tc.expectRule {
...
// TestGetChipPairingRule_ConfigOverride tests that config rules take precedence over hardcoded defaults
func TestGetChipPairingRule_ConfigOverride(t *testing.T) {
	t.Logf("\n=== Testing Config Chip Rules Override ===")

	// Define config rules that override an existing chip and add a new one
	configRules := []ConfigChipRule{
		{
			// Override existing ina226 rule
			Name:         "ina226",
			UseSameIndex: true, // Different from hardcoded (which uses explicit pairings)
			SkipVoltages: []int{0, 2},
		},
		{
			// Add a new chip not in hardcoded list
			Name: "my_custom_chip",
	...
	// Test override: ina226 should use config rule
	rule := getChipPairingRule("ina226", configRules)
	require.NotNil(t, rule)
	assert.True(t, rule.useSameIndex, "ina226 should use useSameIndex from config (not hardcoded)")
	assert.Equal(t, []int{0, 2}, rule.skipVoltages, "ina226 should use skipVoltages from config")
	t.Logf("✓ Config rule correctly overrides hardcoded rule for ina226")

	// Test new chip: my_custom_chip should be found
	rule = getChipPairingRule("my_custom_chip", configRules)
	require.NotNil(t, rule, "Should find config rule for my_custom_chip")
	assert.Equal(t, map[int]int{1: 1, 2: 3}, rule.pairings, "my_custom_chip should have correct pairings")
	assert.Equal(t, []int{0}, rule.skipVoltages, "my_custom_chip should have correct skipVoltages")
	t.Logf("✓ Config rule correctly adds new chip my_custom_chip")

	// Test fallback: ina3221 should still use hardcoded rule (not in config)
	rule = getChipPairingRule("ina3221", configRules)
	require.NotNil(t, rule)
	assert.True(t, rule.useSameIndex, "ina3221 should use hardcoded rule")
	assert.Equal(t, []int{4, 5, 6, 7}, rule.skipVoltages, "ina3221 should use hardcoded skipVoltages")
	t.Logf("✓ Hardcoded rule correctly used for ina3221 (not in config)")

	// Test unknown chip: should return nil
	rule = getChipPairingRule("unknown_chip", configRules)
	assert.Nil(t, rule, "Should not find rule for unknown_chip")
	t.Logf("✓ No rule found for unknown chip")
}
...
func TestChipPairingRule_SkipVoltage(t *testing.T) {
	...
	rule := getChipPairingRule("ina3221", nil)
	require.NotNil(t, rule)
...
func TestChipPairingRule_SkipCurrent(t *testing.T) {
	...
	rule := getChipPairingRule("max34440", nil)
	require.NotNil(t, rule)
...
func TestDiscoverVoltageCurrentZones_INA226ChipRule(t *testing.T) {
	...
	zone := zones[0].(*hwmonVoltageCurrentZone)
	power, err := zone.Power()
	require.NoError(t, err)
...
func TestDiscoverVoltageCurrentZones_INA3221ChipRule(t *testing.T) {
	...
	t.Logf("Found %d zones:", len(zones))
	for _, z := range zones {
		zone := z.(*hwmonVoltageCurrentZone)
		power, err := zone.Power()
		require.NoError(t, err)
...
func TestKnownChipPairings_Coverage(t *testing.T) {
	...
	for _, chip := range expectedChips {
		rule := getChipPairingRule(chip, nil)
