...
// ConfigChipRule defines a user-configurable chip pairing rule.
// This allows users to override hardcoded chip rules or add new chips via configuration.
type ConfigChipRule struct {
...
// WithHwmonChipRules sets user-configurable chip pairing rules.
...
func (h *hwmonPowerMeter) groupZonesByName(zones []device.EnergyZone) []device.EnergyZone {
	...
	for name, zones := range zoneGroups {
		...
		aggregated, _ := device.NewAggregatedZone(zones)
...
type sysfsHwmonReader struct {
	...
	logger          *slog.Logger
	configChipRules []ConfigChipRule // User-configurable chip pairing rules (take precedence over hardcoded)
}
...
func (r *sysfsHwmonReader) createCalculatedZone(
	...
	return &hwmonVoltageCurrentZone{
		name:        zoneName,
		index:       index,
...
type hwmonVoltageCurrentZone struct {
	name        string
	index       int
...
func (z *hwmonVoltageCurrentZone) Index() int {
	return z.index
}

func (z *hwmonVoltageCurrentZone) Path() string {
