...
// VendorType represents different BMC vendor implementations
...
func NewServer(config ServerConfig) *Server {
	...
	if config.EnableTLS {
		server.httpServer = httptest.NewTLSServer(http.HandlerFunc(server.handler))
	} else {
...
func (s *Server) GetTLSCertificate() *tls.Certificate {
	if s.httpServer.TLS != nil && len(s.httpServer.TLS.Certificates) > 0 {
		return &s.httpServer.TLS.Certificates[0]
	}
	return nil
}
...
func (s *Server) handleServiceRoot(w http.ResponseWriter, r *http.Request) {
	...
	response := map[string]interface{}{
		...
		"SessionService": map[string]interface{}{
...
func (s *Server) handleChassis(w http.ResponseWriter, r *http.Request) {
	...
	response := map[string]interface{}{
		...
		"Manufacturer":   string(s.config.Vendor),
		"PowerState":     "On",
...
func (s *Server) handlePower(w http.ResponseWriter, r *http.Request) {
	...
	response := GetPowerResponse(s.config.Vendor, powerWatts)
	_ = json.NewEncoder(w).Encode(response)
}
