...
func TestPowerCollectorConcurrency(t *testing.T) {
	...
	collector := NewPowerCollector(fakeMonitor, "test-node", newLogger(), metrics.MetricsLevelAll)
...
func TestPowerCollectorWithRegistry(t *testing.T) {
	...
	collector := NewPowerCollector(mockMonitor, "test-node", newLogger(), metrics.MetricsLevelAll)
