...
const (
	// MetricsLevelNone represents no metrics enabled
	MetricsLevelNone Level = 0

	// Individual metric levels using bit patterns
	MetricsLevelNode      Level = 1 << (iota - 1) // 1
...
func (l Level) String() string {
	if l == MetricsLevelNone {
		return "none"
	}
	if l == MetricsLevelAll {
		return "all"
	}
...
func ParseLevel(levels []string) (Level, error) {
	...
	for _, level := range levels {
		...
		case "all":
			return MetricsLevelAll, nil
		case "none":
			return MetricsLevelNone, nil
		default:
			return MetricsLevelNone, fmt.Errorf("unknown metrics level: %s", level)
		}
	}
...
func ValidLevels() []string {
	return []string{"node", "process", "container", "vm", "pod", "all", "none"}
}
