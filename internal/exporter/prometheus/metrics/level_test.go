...
func TestParseLevel(t *testing.T) {
	tests := []struct {
		...
		{
			name:        "All with other levels (should return all)",
			levels:      []string{"all", "node"},
			expected:    MetricsLevelAll,
			expectError: false,
		},
	}
...
func TestValidLevels(t *testing.T) {
	expected := []string{"node", "process", "container", "vm", "pod", "all", "none"}
	result := ValidLevels()
...
func TestBitPatterns(t *testing.T) {
	...
	// Test that MetricsLevelAll combines all levels
