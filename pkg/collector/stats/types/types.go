...
func (s *UInt64Stat) SetNewDeltaValue(newDelta uint64, sum bool) error {
	...
	if sum {
		...
		if swapped := s.delta.CompareAndSwap(s.delta.Load(), newDelta); !swapped {
			return fmt.Errorf("the delta value has been changed by another process")
		}
	}
...
func (s *UInt64Stat) SetNewAggr(newAggr uint64) error {
	...
	if (currAggr > 0) && (newAggr > currAggr) {
		if swapped := s.delta.CompareAndSwap(s.delta.Load(), newAggr-currAggr); !swapped {
			return fmt.Errorf("the delta value has been changed by another process")
		}
	}
	if swapped := s.aggr.CompareAndSwap(currAggr, newAggr); !swapped {
		return fmt.Errorf("the aggregated value has been changed by another process")
	}
	return nil
}
...
func (s UInt64StatCollection) AddDeltaStat(key string, newDelta uint64) {
	if instance, found := s[key]; !found {
		stat := &UInt64Stat{}
		stat.aggr.Store(newDelta)
		stat.delta.Store(newDelta)
		s[key] = stat
	} else {
