...
func (e *exporter) attach() error {
	...
	e.schedSwitchLink, err = link.Tracepoint("sched", "sched_switch", e.bpfObjects.KeplerSchedSwitchTrace, nil)
	if err != nil {
	...
	if config.ExposeIRQCounterMetrics {
		e.irqLink, err = link.Tracepoint("irq", "softirq_entry", e.bpfObjects.KeplerIrqTrace, nil)
		if err != nil {
	...
	e.pageWriteLink, err = link.Tracepoint("writeback", "writeback_dirty_folio", e.bpfObjects.KeplerWritePageTrace, nil)
	if err != nil {
