...
oc apply --kustomize $(pwd)/manifests/k8s/config/dashboard
while ! oc get grafana --all-namespaces; do
	echo waiting for grafana custom resource definition to register
	sleep 5
done
oc apply -f $(pwd)/manifests/k8s/config/dashboard/02-grafana-instance.yaml
oc apply -f $(pwd)/manifests/k8s/config/dashboard/02-grafana-sa.yaml
oc apply -f $(pwd)/manifests/k8s/config/dashboard/03-grafana-sa-token-secret.yaml

SERVICE_ACCOUNT=grafana-serviceaccount
SECRET=grafana-sa-token

while ! oc get serviceaccount $SERVICE_ACCOUNT -n kepler; do
	sleep 2
done
...
while ! oc get grafanadatasource --all-namespaces; do
	sleep 1
	echo waiting for grafanadatasource custom resource definition to register
done

# Deploy from updated manifest
envsubst <$(pwd)/manifests/k8s/config/dashboard/03-grafana-datasource-UPDATETHIS.yaml | oc apply -f -

# Define Grafana dashboard
while ! oc get grafanadashboard --all-namespaces; do
	sleep 1
	echo waiting for grafandashboard custom resource definition to register
done

oc apply -f $(pwd)/manifests/k8s/config/dashboard/04-grafana-dashboard.yaml
