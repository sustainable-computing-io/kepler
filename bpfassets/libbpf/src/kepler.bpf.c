...
static inline u64 get_on_cpu_task_clock_time(u32 *cpu_id, u64 cur_ts)
{
	...
	int error = bpf_perf_event_read_value(
		&task_clock_ms_event_reader, *cpu_id, &c,
...
static inline u64 get_on_cpu_cycles(u32 *cpu_id)
{
	...
	struct bpf_perf_event_value c = {};
	int error = bpf_perf_event_read_value(
		&cpu_cycles_event_reader, *cpu_id, &c,
		sizeof(struct bpf_perf_event_value));
	if (error == 0) {
		u64 val = c.counter;
...
static inline u64 get_on_cpu_ref_cycles(u32 *cpu_id)
{
	...
	struct bpf_perf_event_value c = {};
	int error = bpf_perf_event_read_value(
		&cpu_ref_cycles_event_reader, *cpu_id, &c,
		sizeof(struct bpf_perf_event_value));
	if (error == 0) {
		u64 val = c.counter;
...
static inline u64 get_on_cpu_instr(u32 *cpu_id)
{
	...
	struct bpf_perf_event_value c = {};
	int error = bpf_perf_event_read_value(
		&cpu_instructions_event_reader, *cpu_id, &c,
		sizeof(struct bpf_perf_event_value));
	if (error == 0) {
		u64 val = c.counter;
...
static inline u64 get_on_cpu_cache_miss(u32 *cpu_id)
{
	...
	struct bpf_perf_event_value c = {};
	int error = bpf_perf_event_read_value(
		&cache_miss_event_reader, *cpu_id, &c,
		sizeof(struct bpf_perf_event_value));
	if (error == 0) {
		u64 val = c.counter;
