...
static inline u64
get_on_cpu_time(pid_t cur_pid, u32 prev_pid, u32 cpu_id, u64 cur_ts)
{
	u64 cpu_time = 0;
...
// altough the "get_on_cpu_counters" has some code duplications, it is inline
// code and the compiles will improve this
static inline u64 get_on_cpu_cycles(u32 *cpu_id)
{
	u64 delta = 0;
	struct bpf_perf_event_value c = {};
	int error = cpu_cycles_hc_reader.perf_counter_value(
		CUR_CPU_IDENTIFIER, &c, sizeof(struct bpf_perf_event_value));
	if (error == 0) {
...
static inline u64 get_on_cpu_ref_cycles(u32 *cpu_id)
{
	u64 delta = 0;
	struct bpf_perf_event_value c = {};
	int error = cpu_ref_cycles_hc_reader.perf_counter_value(
		CUR_CPU_IDENTIFIER, &c, sizeof(struct bpf_perf_event_value));
	if (error == 0) {
		u64 val = normalize(&c.counter, &c.enabled, &c.running);
...
static inline u64 get_on_cpu_instr(u32 *cpu_id)
{
	u64 delta = 0;
	struct bpf_perf_event_value c = {};
	int error = cpu_instructions_hc_reader.perf_counter_value(
		CUR_CPU_IDENTIFIER, &c, sizeof(struct bpf_perf_event_value));
	if (error == 0) {
		u64 val = normalize(&c.counter, &c.enabled, &c.running);
...
static inline u64 get_on_cpu_cache_miss(u32 *cpu_id)
{
	u64 delta = 0;
	struct bpf_perf_event_value c = {};
	int error = cache_miss_hc_reader.perf_counter_value(
		CUR_CPU_IDENTIFIER, &c, sizeof(struct bpf_perf_event_value));
	if (error == 0) {
		u64 val = normalize(&c.counter, &c.enabled, &c.running);
...
static inline u64 get_on_cpu_avg_freq(
	u32 *cpu_id, u64 on_cpu_cycles_delta, u64 on_cpu_ref_cycles_delta)
{
	u32 avg_freq = 0;
	cpu_freq_array.lookup_or_try_init(cpu_id, &avg_freq);
	if (avg_freq == 0) {
		avg_freq = ((on_cpu_cycles_delta * CPU_REF_FREQ) /
...
{
	u32 initial = SAMPLE_RATE, *counter_sched_switch,
	    sample_counter_key = 1234;
	// only do sampling if sample rate is set
	if (initial != 0) {
		counter_sched_switch = sample_rate.lookup_or_try_init(
			&sample_counter_key, &initial);
	...
	u64 on_cpu_avg_freq = get_on_cpu_avg_freq(
		&cpu_id, on_cpu_cycles_delta, on_cpu_ref_cycles_delta);
...
// per
// https://www.kernel.org/doc/html/latest/core-api/tracepoint.html#c.trace_softirq_entry
