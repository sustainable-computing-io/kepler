/*
Copyright 2021.

Licensed under the Apache License, Version 2.0 (the "License");
you may not use this file except in compliance with the License.
You may obtain a copy of the License at

    http://www.apache.org/licenses/LICENSE-2.0

Unless required by applicable law or agreed to in writing, software
distributed under the License is distributed on an "AS IS" BASIS,
WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
See the License for the specific language governing permissions and
limitations under the License.
*/
// #include <linux/types.h>
// #include <linux/bpf.h>
// #include <linux/bpf_perf_event.h>
// #include <linux/perf_event.h>
// need to install libbpf-devel to get bpf_helpers.h

#ifndef __BCC__
#include "vmlinux.h"
#include <bpf/bpf_helpers.h>
#endif
#include <linux/sched.h>


#ifndef NUM_CPUS
#define NUM_CPUS 128
#endif

#ifndef CPU_REF_FREQ
#define CPU_REF_FREQ 2500
#endif

#ifndef HZ
#define HZ 1000
#endif

#ifndef MAP_SIZE
#define MAP_SIZE 10240
#endif

typedef struct process_metrics_t
{
    u64 cgroup_id;
    u64 pid;
    u64 process_run_time;
    u64 cpu_cycles;
    u64 cpu_instr;
    u64 cache_miss;
    u16 vec_nr[10]; // irq counter, 10 is the max number of irq vectors
    char comm[16];
} process_metrics_t;

typedef struct pid_time_t
{
    u32 pid;
    u32 cpu;
} pid_time_t;

// processes and pid time
#ifndef __BCC__
struct bpf_map_def SEC("maps/processes") processes = {
	.type = BPF_MAP_TYPE_HASH,
	.key_size = sizeof(process_metrics_t),
	.value_size = sizeof(u64),
	.max_entries = MAP_SIZE,
} ;

struct bpf_map_def SEC("maps/pid_time") pid_time = {
	.type = BPF_MAP_TYPE_HASH,
	.key_size = sizeof(pid_time_t),
	.value_size = sizeof(u64),
	.max_entries = MAP_SIZE,
} ;
#else
BPF_HASH(processes, u64, process_metrics_t, MAP_SIZE);
BPF_HASH(pid_time, pid_time_t);
#endif

// perf counters
#ifndef __BCC__
struct bpf_map_def SEC("maps/cpu_cycles_hc_reader") cpu_cycles_hc_reader = {
	.type = BPF_MAP_TYPE_PERF_EVENT_ARRAY,
	.key_size = sizeof(int),
	.value_size = sizeof(u32),
	.max_entries = NUM_CPUS,
};

struct bpf_map_def SEC("maps/cpu_cycles") cpu_cycles = {
	.type = BPF_MAP_TYPE_HASH,
	.key_size = sizeof(int),
	.value_size = sizeof(u64),
	.max_entries = NUM_CPUS,
};

struct bpf_map_def SEC("maps/cpu_ref_cycles_hc_reader") cpu_ref_cycles_hc_reader = {
	.type = BPF_MAP_TYPE_PERF_EVENT_ARRAY,
	.key_size = sizeof(int),
	.value_size = sizeof(u32),
	.max_entries = NUM_CPUS,
};

struct bpf_map_def SEC("maps/cpu_ref_cycles") cpu_ref_cycles = {
	.type = BPF_MAP_TYPE_HASH,
	.key_size = sizeof(int),
	.value_size = sizeof(u64),
	.max_entries = NUM_CPUS,
};

struct bpf_map_def SEC("maps/cpu_instr_hc_reader") cpu_instr_hc_reader = {
	.type = BPF_MAP_TYPE_PERF_EVENT_ARRAY,
	.key_size = sizeof(int),
	.value_size = sizeof(u32),
	.max_entries = NUM_CPUS,
};

struct bpf_map_def SEC("maps/cpu_instr") cpu_instr = {
	.type = BPF_MAP_TYPE_HASH,
	.key_size = sizeof(int),
	.value_size = sizeof(u64),
	.max_entries = NUM_CPUS,
};

struct bpf_map_def SEC("maps/cache_miss_hc_reader") cache_miss_hc_reader = {
	.type = BPF_MAP_TYPE_PERF_EVENT_ARRAY,
	.key_size = sizeof(int),
	.value_size = sizeof(u32),
	.max_entries = NUM_CPUS,
};

struct bpf_map_def SEC("maps/cache_miss")  cache_miss = {
	.type = BPF_MAP_TYPE_HASH,
	.key_size = sizeof(int),
	.value_size = sizeof(u64),
	.max_entries = NUM_CPUS,
};

struct bpf_map_def SEC("maps/cpu_freq_array") cpu_freq_array = {
	.type = BPF_MAP_TYPE_HASH,
	.key_size = sizeof(int),
	.value_size = sizeof(u64),
	.max_entries = NUM_CPUS,
};

#else
BPF_PERF_ARRAY(cpu_cycles_hc_reader, NUM_CPUS);
BPF_ARRAY(cpu_cycles, u64, NUM_CPUS);
BPF_PERF_ARRAY(cpu_ref_cycles_hc_reader, NUM_CPUS);
BPF_ARRAY(cpu_ref_cycles, u64, NUM_CPUS);
BPF_PERF_ARRAY(cpu_instr_hc_reader, NUM_CPUS);
BPF_ARRAY(cpu_instr, u64, NUM_CPUS);
BPF_PERF_ARRAY(cache_miss_hc_reader, NUM_CPUS);
BPF_ARRAY(cache_miss, u64, NUM_CPUS);
BPF_ARRAY(cpu_freq_array, u32, NUM_CPUS);
#endif

static inline u64 get_on_cpu_time(u32 cur_pid, u32 prev_pid, u32 cpu_id, u64 cur_ts)
{
    u64 cpu_time = 0;

    // get pid time
    pid_time_t prev_pid_key = {.pid = prev_pid, .cpu = cpu_id};
    #ifdef __BCC__
    u64 *prev_ts = pid_time.lookup(&prev_pid_key); // BCC
    #else
    u64 *prev_ts = bpf_map_lookup_elem(&pid_time, &prev_pid_key);
    #endif

    if (prev_ts != 0)
    {
        // Probably a clock issue where the recorded on-CPU event had a
        // timestamp later than the recorded off-CPU event, or vice versa.
        // But do not return, since the hardware counters can be collected.
        if (cur_ts > *prev_ts)
        {
            cpu_time = (cur_ts - *prev_ts) / 1000000; /*milisecond*/
            #ifdef __BCC__
            pid_time.delete(&prev_pid_key); // BCC
            #else
            bpf_map_delete_elem(&pid_time, &prev_pid_key);
            #endif
        }
    }
    pid_time_t new_pid_key = {.pid = cur_pid, .cpu = cpu_id};
    #ifdef __BCC__
    pid_time.update(&new_pid_key, &cur_ts); // BCC
    #else
    bpf_map_update_elem(&pid_time, &new_pid_key, &cur_ts, BPF_ANY);
    #endif

    return cpu_time;
}

static inline u64 normalize(u64 *counter, u64 *enabled, u64 *running)
{
    if (*running > 0)
        return *counter * *enabled / *running;
    return *counter;
}

static inline u64 calc_delta(u64 *prev_val, u64 *val)
{
    u64 delta = 0;
    if (prev_val)
    {
        if (*val > *prev_val)
            delta = *val - *prev_val;
    }
    return delta;
}

// altough the "get_on_cpu_counters" has some code duplications, it is inline code and the compiles will improve this
static inline u64 get_on_cpu_cycles(u32 *cpu_id)
{
    u64 delta = 0;
    struct bpf_perf_event_value c = {};
    u32 cpu = bpf_get_smp_processor_id();
    #ifdef __BCC__
    int error = cpu_cycles_hc_reader.perf_counter_value(CUR_CPU_IDENTIFIER, &c, sizeof(struct bpf_perf_event_value)); // BCC
    #else
    int error = bpf_perf_event_read_value(&cpu_cycles_hc_reader, cpu, &c, sizeof(struct bpf_perf_event_value));
    #endif

    if (error == 0)
    {
        u64 val = normalize(&c.counter, &c.enabled, &c.running);
        #ifdef __BCC__
        u64 *prev_val = cpu_cycles.lookup(cpu_id); // BCC
        #else
        u64 *prev_val = bpf_map_lookup_elem(&cpu_cycles, cpu_id);
        #endif

        delta = calc_delta(prev_val, &val);
        #ifdef __BCC__
        cpu_cycles.update(cpu_id, &val); // BCC
        #else
        bpf_map_update_elem(&cpu_cycles, cpu_id, &val, BPF_ANY);
        #endif
    }
    return delta;
}

static inline u64 get_on_cpu_ref_cycles(u32 *cpu_id)
{
    u64 delta = 0;
    struct bpf_perf_event_value c = {};
    u32 cpu = bpf_get_smp_processor_id();
    #ifdef __BCC__
    int error = cpu_ref_cycles_hc_reader.perf_counter_value(CUR_CPU_IDENTIFIER, &c, sizeof(struct bpf_perf_event_value)); // BCC
    #else
    int error = bpf_perf_event_read_value(&cpu_ref_cycles_hc_reader, cpu, &c, sizeof(struct bpf_perf_event_value));
    #endif

    if (error == 0)
    {
        u64 val = normalize(&c.counter, &c.enabled, &c.running);
        #ifdef __BCC__
        u64 *prev_val = cpu_ref_cycles.lookup(cpu_id); // BCC
        #else
        u64 *prev_val = bpf_map_lookup_elem(&cpu_ref_cycles, cpu_id);
        #endif
        delta = calc_delta(prev_val, &val);

        #ifdef __BCC__
        cpu_ref_cycles.update(cpu_id, &val); // BCC
        #else
        bpf_map_update_elem(&cpu_ref_cycles, cpu_id, &val, BPF_ANY);
        #endif
    }
    return delta;
}

static inline u64 get_on_cpu_instr(u32 *cpu_id)
{
    u64 delta = 0;
    struct bpf_perf_event_value c = {};
    u32 cpu = bpf_get_smp_processor_id();
    #ifdef __BCC__
    int error = cpu_instr_hc_reader.perf_counter_value(CUR_CPU_IDENTIFIER, &c, sizeof(struct bpf_perf_event_value)); // BCC
    #else
    int error = bpf_perf_event_read_value(&cpu_instr_hc_reader, cpu, &c, sizeof(struct bpf_perf_event_value));
    #endif

    if (error == 0)
    {
        u64 val = normalize(&c.counter, &c.enabled, &c.running);
        #ifdef __BCC__
        u64 *prev_val = cpu_instr.lookup(cpu_id); // BCC
        #else
        u64 *prev_val = bpf_map_lookup_elem(&cpu_instr, cpu_id);
        #endif

        delta = calc_delta(prev_val, &val);
        #ifdef __BCC__
        cpu_instr.update(cpu_id, &val); // BCC
        #else
        bpf_map_update_elem(&cpu_instr, cpu_id, &val, BPF_ANY);
        #endif
    }
    return delta;
}

static inline u64 get_on_cpu_cache_miss(u32 *cpu_id)
{
    u64 delta = 0;
    struct bpf_perf_event_value c = {};
    u32 cpu = bpf_get_smp_processor_id();
    #ifdef __BCC__
    int error = cache_miss_hc_reader.perf_counter_value(CUR_CPU_IDENTIFIER, &c, sizeof(struct bpf_perf_event_value)); // BCC
    #else
    int error = bpf_perf_event_read_value(&cache_miss_hc_reader, cpu, &c, sizeof(struct bpf_perf_event_value));
    #endif

    if (error == 0)
    {
        u64 val = normalize(&c.counter, &c.enabled, &c.running);
        #ifdef __BCC__
        u64 *prev_val = cache_miss.lookup(cpu_id); // BCC
        #else
        u64 *prev_val = bpf_map_lookup_elem(&cache_miss, cpu_id);
        #endif
        delta = calc_delta(prev_val, &val);
        #ifdef __BCC__
        cache_miss.update(cpu_id, &val); // BCC
        #else
        bpf_map_update_elem(&cache_miss, cpu_id, &val, BPF_ANY);
        #endif
    }
    return delta;
}

// calculate the average cpu freq
static inline u64 get_on_cpu_avg_freq(u32 *cpu_id, u64 on_cpu_cycles_delta, u64 on_cpu_ref_cycles_delta)
{
    u32 avg_freq = 0;
    #ifdef __BCC__
    cpu_freq_array.lookup_or_try_init(cpu_id, &avg_freq); // BCC
    #else
    struct bpf_perf_event_value *val = bpf_map_lookup_elem(&cpu_freq_array, cpu_id);
    if (val)
    {
        avg_freq = val->counter;
    }
    else
    {
        bpf_map_update_elem(&cpu_freq_array, cpu_id, &avg_freq, BPF_ANY);
    }
    #endif
    if (avg_freq == 0)
    {
        avg_freq = ((on_cpu_cycles_delta * CPU_REF_FREQ) / on_cpu_ref_cycles_delta) * HZ;
    }
    else
    {
        avg_freq += ((on_cpu_cycles_delta * CPU_REF_FREQ) / on_cpu_ref_cycles_delta) * HZ;
        avg_freq /= 2;
    }
    return avg_freq;
}

#ifndef __BCC__
SEC("kprobe/finish_task_switch")
#endif
int kprobe__finish_task_switch(struct pt_regs *ctx, struct task_struct *prev)
{
    u64 cur_pid = bpf_get_current_pid_tgid() >> 32;
#ifdef SET_GROUP_ID
    u64 cgroup_id = bpf_get_current_cgroup_id();
#else
    u64 cgroup_id = 0;
#endif

    u64 cur_ts = bpf_ktime_get_ns();
    u32 cpu_id = bpf_get_smp_processor_id();
    u64 prev_pid = prev->pid;
    u64 on_cpu_time_delta = get_on_cpu_time(cur_pid, prev_pid, cpu_id, cur_ts);
    u64 on_cpu_cycles_delta = get_on_cpu_cycles(&cpu_id);
    u64 on_cpu_ref_cycles_delta = get_on_cpu_ref_cycles(&cpu_id);
    u64 on_cpu_instr_delta = get_on_cpu_instr(&cpu_id);
    u64 on_cpu_cache_miss_delta = get_on_cpu_cache_miss(&cpu_id);
    u64 on_cpu_avg_freq = get_on_cpu_avg_freq(&cpu_id, on_cpu_cycles_delta, on_cpu_ref_cycles_delta);

    // store process metrics
    struct process_metrics_t *process_metrics;
    #ifdef __BCC__
    process_metrics = processes.lookup(&prev_pid); // BCC
    #else
    process_metrics = bpf_map_lookup_elem(&processes, &prev_pid);
    #endif
    if (process_metrics != 0)
    {
        // update process time
        process_metrics->process_run_time += on_cpu_time_delta;

        process_metrics->cpu_cycles += on_cpu_cycles_delta;
        process_metrics->cpu_instr += on_cpu_instr_delta;
        process_metrics->cache_miss += on_cpu_cache_miss_delta;
    }
    #ifdef __BCC__
    process_metrics = processes.lookup(&cur_pid); // BCC
    #else
    process_metrics = bpf_map_lookup_elem(&processes, &cur_pid);
    #endif
    if (process_metrics == 0)
    {
        process_metrics_t new_process = {};
        new_process.pid = cur_pid;
        new_process.cgroup_id = cgroup_id;
        bpf_get_current_comm(&new_process.comm, sizeof(new_process.comm));
        #ifdef __BCC__
        processes.update(&cur_pid, &new_process); // BCC
        #else
        bpf_map_update_elem(&processes, &cur_pid, &new_process, BPF_ANY);
        #endif
    }

    return 0;
}

// per https://www.kernel.org/doc/html/latest/core-api/tracepoint.html#c.trace_softirq_entry
#ifndef __BCC__
struct trace_event_raw_softirq_entry {
        unsigned short common_type;
        unsigned char common_flags;
        unsigned char common_preempt_count;
        int common_pid;
        unsigned int vec;
};
SEC("tracepoint/irq/softirq_entry")
int tracepoint__irq__softirq_entry(struct trace_event_raw_softirq_entry *args)
#else
TRACEPOINT_PROBE(irq, softirq_entry) // BCC
#endif
{
    u64 cur_pid = bpf_get_current_pid_tgid() >> 32;
    struct process_metrics_t *process_metrics;
    #ifdef __BCC__
    process_metrics = processes.lookup(&cur_pid); // BCC
    #else
    process_metrics = bpf_map_lookup_elem(&processes, &cur_pid);
    #endif
    if (process_metrics != 0)
    {
        if (args->vec < 10) {
            process_metrics->vec_nr[args->vec] ++;
        }
    }
    return 0;
}
#ifndef __BCC__
char _license[] SEC("license") = "GPL";
#endif