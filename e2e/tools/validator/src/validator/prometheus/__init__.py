...
class MetricsValidatorResult2(NamedTuple):
    # mean absolute error
    mae: float
...
class MetricsValidator:
    ...
    def compare_metrics2(self, start_time: datetime, 
                        end_time: datetime, 
                        expected_query: str, 
                        actual_query: str, 
                        ) -> None:   
        ...
        cleaned_expected_metrics = retrieve_timestamp_value_metrics2(expected_metrics[0])
        cleaned_actual_metrics = retrieve_timestamp_value_metrics2(actual_metrics[0])
        ...
        print(expected_data)
        print(actual_data)
        return MetricsValidatorResult2(
...
def retrieve_timestamp_value_metrics2(prom_query_response) -> List[float]: #List[List[Tuple[int, float]]]:
    acquired_data = []
    for element in prom_query_response['values']:
...
def absolute_percentage_error(expected_data, actual_data) -> List[float]:
    ...
    absolute_percentage_error = np.abs((expected_data - actual_data) / expected_data)
