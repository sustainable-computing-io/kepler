...
The solution uses a simple and explicit node identification approach:

**Node ID Configuration**: Nodes are identified using the `--platform.node-id` CLI flag or
the `platform.nodeID` YAML configuration value. This identifier must be explicitly provided
and matches the node identifiers in the BMC configuration file.

**Examples:**

- CLI: `--platform.node-id=worker-node-1`
- YAML: `platform: { nodeID: "worker-node-1" }`

This creates a single BMC configuration file that maps node identifiers to BMC configurations,
requiring explicit node identification for clarity and predictability.
...
./kepler --platform.node-id=my-server --platform.redfish.enabled=true
```
