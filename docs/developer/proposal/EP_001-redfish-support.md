...
Redfish and integrates with the existing PowerMonitor for unified power attribution.

```text
┌─────────────────┐    ┌──────────────────┐    ┌─────────────────┐
│   CPU Power     │    │  Platform Power  │    │  Power Monitor  │
│   (RAPL)        │────│   (Redfish)      │────│   (Aggregator)  │
│                 │    │                  │    │                 │
└─────────────────┘    └──────────────────┘    └─────────────────┘
                                │
                                ▼
...
1. **CLI Override**: `--platform.node-identifier=my-node` (highest priority)
...
type Platform struct {
    NodeIdentifier string  `yaml:"nodeIdentifier"`  // High-level node identifier
...
```bash
--platform.node-identifier=worker-node-1             # Node identifier override
...
- Integrate with PowerMonitor for unified power attribution

### Phase 3: Metrics and Export

- Create platform power Prometheus collector
...
**Rejected**: Legacy protocol with vendor-specific implementations. Redfish provides standardized REST API.

### Direct Hardware Integration

**Rejected**: Would require per-vendor implementations. BMC integration leverages existing infrastructure.

### External Power Monitoring Systems

**Deferred**: Adds deployment complexity. Redfish provides native integration with existing BMC infrastructure.
...
3. **Historical Data**: Should platform power contribute to workload power attribution models?
4. **Integration Scope**: Should this proposal include power capping controls or remain monitoring-only?
